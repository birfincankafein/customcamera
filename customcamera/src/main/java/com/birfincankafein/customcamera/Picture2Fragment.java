/*
 * Modified by metehantoksoy, originally developed by The Android Open Source Project
...
public class Picture2Fragment extends Fragment {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), saveFileUri, getActivity()));
        }
    };
    ...
    public static Picture2Fragment newInstance() {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (facing != null) {
                    if (facing == CameraCharacteristics.LENS_FACING_FRONT && !cameraState.equals(CameraState.FRONT)) {
                        continue;
                    } else if (facing == CameraCharacteristics.LENS_FACING_BACK && !cameraState.equals(CameraState.REAR)) {
                        continue;
                ...
                if (resolutionWidth == -1 || resolutionHeight == -1) {
                    ...
                    largest = new Size(resolutionWidth, resolutionHeight);
                }
                ...
                switch (displayRotation) {
                    ...
                    case Surface.ROTATION_270:
                        ...
                        break;
                    default:
                        Log.e(CameraActivity.TAG, "Display rotation is invalid: " + displayRotation);
                }
    ...
    /**
     * Opens the camera specified by {@link Picture2Fragment#mCameraId}.
     */
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               @NonNull CaptureRequest request,
                                               @NonNull TotalCaptureResult result) {
                    showToast("Saved: " + saveFileUri.getPath());
                    Log.d(CameraActivity.TAG, saveFileUri.toString());
    ...
    private void setAutoFlash(CaptureRequest.Builder requestBuilder) {
        // Check if flash supported and flash enabled
        if (mFlashSupported) {
            switch (flashState) {
                case ON:
    ...
    private static class ImageSaver implements Runnable {
        ...
        ImageSaver(Image image, Uri fileUri, Context context) {
            mImage = image;
            mFileUri = fileUri;
            mContext = context;
        }
        ...
        public void run() {
            ...
            try {
                output = mContext.getContentResolver().openOutputStream(mFileUri);
                output.write(bytes);
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
